...
- Do not promote ordinary newly processed notes directly into `Core Topics`. Treat that as rare and reserved for notes that are already durable cluster entrypoints under the content-index rule.
- If `Notes/Index.md` is updated during triage, choose the section intentionally:
  - `Core Topics` only for durable cluster entrypoints
  - `Synthesis And Overviews` for bridge notes, comparisons, and non-top-level overviews
  - bootstrap only for temporary top-level discoverability
...
- confirm any direct index update used the correct section instead of promoting a non-durable note into `Core Topics`
